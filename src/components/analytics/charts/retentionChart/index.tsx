...
import { DatePicker, Form, Select } from "@pankod/refine-antd";
...
export const RetentionChart: React.FC<{
    ...
    }, [dateRange, communityIds, groupByFilter])

    useEffect(() => {
        if (!isLoading && (data || initialData)) {
            const responseData = data?.data || initialData;
    ...
    }, [data, isLoading, initialData, groupByFilter]);
    ...
    function transformData(inputData: RetentionAnalyticsQuery[]) {
        ...
        sortedDates.forEach((dateKey, index) => {
            ...
            const retentionRate = prevIntervalTotal > 0 ? ((retainedEntriesTotal / prevIntervalTotal) * 100).toFixed(1) : '0.0';
            ...
            plots.push({
                ...
                options: {
                    ...
                    annotations: [
                        {
                            type: 'text',
                            content: data && data[0] ? getDateFormat(data[0].month) : '',
                            position: ['50%', '0%'],
                            offsetY: -15,
                            style: {
                                textAlign: 'center',
